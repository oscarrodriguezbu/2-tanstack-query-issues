...
export const IssueItem = ({ issue }: Props) => {
  ...
  const queryClient = useQueryClient(); // esto va y busca en el context y retorna el queryClient

  const prefetchData = () => {
    queryClient.prefetchQuery({// se usa para disparar algo, en este caso la peticion http
      queryKey: ['issues', issue.number],
      queryFn: () => getIssue(issue.number),
      staleTime: 1000 * 60, //esto evita disparar un monton de peticiones al pasar el mouse por encima como loco
    });
